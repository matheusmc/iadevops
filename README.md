...
De forma resumida, o desenvolverdor faz a o CI/CD na versão web do Gitlab que através da configuração do Gitlab Runner executa um container que faz a implantação da aplicação dentro da máquina de destino. A aplicação de teste está rodando na porta 80 e pode se acessada através do link: http://site.matheusmc.com.br. Além da aplícação de teste, nessa máquina estão rodando as aplicações de monitoramento (prometheus, grafana, cadvasor, node exporte). A figura abaixo representa a estrutura lógica da infra utilizada:
